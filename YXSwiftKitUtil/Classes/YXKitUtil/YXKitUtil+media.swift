...
public extension YXSwiftKitUtil {
    ...
    @discardableResult
    func playMusic(url: URL?, repeated: Bool = false, audioSessionCategory: AVAudioSession.Category = AVAudioSessionCategoryPlayback as AVAudioSession.Category) -> AVAudioPlayer? {
        guard var musicURL = url else { return nil }
        audioPlayer?.stop()
        
        let audioSession = AVAudioSession.sharedInstance()
        try? audioSession.setCategory(audioSessionCategory as String)
        try? audioSession.setActive(true, with: AVAudioSession.SetActiveOptions.init())
        
        
        if musicURL.absoluteString.hasPrefix("http://") || musicURL.absoluteString.hasPrefix("https://") {
            guard let name = musicURL.path.yx.encryptString(encryType: .md5) else { return nil }
            let path = YXSwiftKitUtil.shared.createFileDirectory(in: .caches, directoryName: "music").appendingPathComponent(name, isDirectory: false)
