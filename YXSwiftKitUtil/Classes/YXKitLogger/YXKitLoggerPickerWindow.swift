...
class YXKitLoggerPickerWindow: UIWindow {
    @available(iOS 13.0, *)
    ...
    private var pickerType: PickerType = .share {
        willSet {
            if newValue == .share {
                mPickerTipLabel.text = "Please select the log to share"
            } else if newValue == .upload {
                mPickerTipLabel.text = "Please select the log to upload"
            }
        }
    }
    ...
    private lazy var mPickerTipLabel: UILabel = {
        ...
        tipLabel.text = "Please select the log to share"
...
extension YXKitLoggerPickerWindow {
...
private extension YXKitLoggerPickerWindow {
    func _initVC() {
    ...
    private func _createUI() {
        ...
        let doneBarItem = UIBarButtonItem(title: "Done", style:.plain, target: self, action: #selector(_confirmPicker))
    ...
    @objc private func _confirmPicker() {
        if self.pickerType == .share {
            ...
            //写入到text文件好解析
            //文件路径
            let logFilePathURL = YXSwiftKitUtil.shared.getFileDirectory(type: .caches).appendingPathComponent("YXKitLogger.log", isDirectory: false)
            ...
            if UIDevice.current.model == "iPad" {
                ...
                let view: UIView = YXSwiftKitUtil.shared.getCurrentVC()?.view ?? self.rootViewController!.view
            ...
            YXSwiftKitUtil.shared.getCurrentVC()?.present(activityVC, animated: true, completion: nil)
        } else if let complete = YXKitLogger.uploadComplete {
...
extension YXKitLoggerPickerWindow: UIPickerViewDelegate, UIPickerViewDataSource {
