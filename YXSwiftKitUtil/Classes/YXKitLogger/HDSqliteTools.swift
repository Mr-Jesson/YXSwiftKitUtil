...
class HDSqliteTools {
    ...
    func getDBFolder() -> URL {
        let dbFolder = YXKitLogger.userID.yx.encryptString(encryType: .md5) ?? "YXKitLog"
        let path = YXSwiftKitUtil.shared.createFileDirectory(in: .documents, directoryName: dbFolder)
        return path
    }

    //插入数据
    func insertLog(log: YXKitLoggerItem) {
