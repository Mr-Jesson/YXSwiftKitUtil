...
public struct YXKitLogType : OptionSet {
    public static let debug = YXKitLogType([])        //only show in debug output
    public static let info = YXKitLogType(rawValue: 1)    //textColor #50d890
    public static let warn = YXKitLogType(rawValue: 2)         //textColor #f6f49d
    public static let error = YXKitLogType(rawValue: 3)        //textColor #ff7676
    public static let privacy = YXKitLogType(rawValue: 4)      //textColor #42e6a4
...
public func printLog(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.debug, file:file, funcName:funcName, lineNum:lineNum)
}
public func printLog(_ log:Any ..., file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.debug, file:file, funcName:funcName, lineNum:lineNum)
}
...
public func printInfo(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.info, file:file, funcName:funcName, lineNum:lineNum)
}
public func printInfo(_ log:Any ..., file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.info, file:file, funcName:funcName, lineNum:lineNum)
}
...
public func printWarn(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.warn, file:file, funcName:funcName, lineNum:lineNum)
}
public func printWarn(_ log:Any ..., file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.warn, file:file, funcName:funcName, lineNum:lineNum)
}
...
public func printError(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.error, file:file, funcName:funcName, lineNum:lineNum)
}
public func printError(_ log:Any ..., file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.error, file:file, funcName:funcName, lineNum:lineNum)
}
...
public func printPrivacy(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.privacy, file:file, funcName:funcName, lineNum:lineNum)
}
public func printPrivacy(_ log:Any ..., file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
    YXKitLogger.printLog(log: log, logType: YXKitLogType.privacy, file:file, funcName:funcName, lineNum:lineNum)
}
...
public func YXDebugLog(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
...
public func YXNormalLog(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
...
public func YXWarnLog(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
...
public func YXErrorLog(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
...
public func YXPrivacyLog(_ log:Any, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
...
public class YXKitLogger {
    ...
    public static var privacyLogPassword = "12345678901234561234567890123456" {
        willSet {
            assert(newValue.count == kCCKeySizeAES256, "The password requires 32 characters")
        }
    }

    //MARK: Private
    private let mFPSTools = YXKitFPS()
    private lazy var loggerWindow: YXKitLoggerWindow? = {
        var window: YXKitLoggerWindow?
        if #available(iOS 13.0, *) {
            for windowScene:UIWindowScene in ((UIApplication.shared.connectedScenes as? Set<UIWindowScene>)!) {
                if windowScene.activationState == .foregroundActive {
                    window = YXKitLoggerWindow(windowScene: windowScene)
                }
            }
        }
        if window == nil {
            window = YXKitLoggerWindow(frame: CGRect.zero)
        }
        return window
    }()
    private lazy var pickerWindow: YXKitLoggerPickerWindow? = {
        var window: YXKitLoggerPickerWindow?
        if #available(iOS 13.0, *) {
            for windowScene:UIWindowScene in ((UIApplication.shared.connectedScenes as? Set<UIWindowScene>)!) {
                if windowScene.activationState == .foregroundActive {
                    window = YXKitLoggerPickerWindow(windowScene: windowScene)
                }
            }
        }
        if window == nil {
            window = YXKitLoggerPickerWindow(frame: CGRect.zero)
        }
        return window
    }()
    private var floatWindow: YXKitLoggerFloatWindow?
    var isPasswordCorrect: Bool = false
    static let shared = YXKitLogger()
    private let logQueue = DispatchQueue(label:"com.YXKitLogger.logQueue", qos:.utility, attributes:.concurrent)
    //是否显示屏幕FPS状态
            ...
            if newValue {
                shared.mFPSTools.start { (fps) in
                    DispatchQueue.main.async {
                        ...
                        if fps >= 55 {
                            shared.floatWindow?.mButton.backgroundColor = UIColor.yx.color(hexValue: 0x5dae8b)
                        } else if (fps >= 50 && fps < 55) {
                            shared.floatWindow?.mButton.backgroundColor = UIColor.yx.color(hexValue: 0xf0a500)
                        } else {
                            shared.floatWindow?.mButton.backgroundColor = UIColor.yx.color(hexValue: 0xaa2b1d)
                        }
                    }
                }
            } else {
                DispatchQueue.main.async {
                    ...
                    shared.floatWindow?.mButton.backgroundColor = UIColor.yx.color(hexString: "#5dae8b")
                    shared.floatWindow?.mButton.setTitle("张", for: UIControl.State.normal)
                }
            }
        }
    }
    ...
    public class func printLog(log:Any, logType:YXKitLogType, file:String = #file, funcName:String = #function, lineNum:Int = #line) -> Void {
...
//            YXWarnLog(NSLocalizedString("The float button already exists", comment: ""))
            #else
            //float window
            if let window = self.shared.floatWindow {
                ...
                if #available(iOS 13.0, *) {
                    for windowScene:UIWindowScene in ((UIApplication.shared.connectedScenes as? Set<UIWindowScene>)!) {
                        if windowScene.activationState == .foregroundActive {
                            self.shared.floatWindow = YXKitLoggerFloatWindow(windowScene: windowScene)
                ...
                if self.shared.floatWindow == nil {
                    self.shared.floatWindow = YXKitLoggerFloatWindow(frame: CGRect(x: UIScreen.main.bounds.size.width - 80, y: 100, width: 60, height: 60))
                }
    ...
    //MARK: init
    init() {
        if YXKitLogger.logExpiryDay > 0 {
            self._checkValidity()
        }
    }
}

private extension YXKitLogger {
    func _writeDB(log: YXKitLoggerItem) -> Void {
